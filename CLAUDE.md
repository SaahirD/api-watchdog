...
- **`oasdiff` itself is unreliable, and not purely as a function of diff-window size:** confirmed directly (2026-08-30) across two separate attempts to force a real spec-diff-to-PR test — first a ~3-month-old checkpoint, then (after the `id`/cap fixes above, to rule those out as the cause) a much smaller ~10-day-old one specifically to avoid the large-window case. `--allow-external-refs=false` (see `stripe_spec_monitor.py`'s `_run_oasdiff`) fixes a genuine hang from external `$ref` resolution, and `MAX_RAW_ENTRIES_PER_RUN` bounds how much *this codebase* processes after oasdiff returns, but `oasdiff`'s own runtime on real Stripe spec pairs still ranged unpredictably: 17.7s once, a genuine hang another time (confirmed via `Get-NetTCPConnection`: zero open connections, zero CPU), 90+ seconds of heavy multi-core computation another time, and — the ~10-day attempt — a full 120s timeout with no output, on a window that should have been small. This suggests the instability isn't (only) about window size — possibly something about Stripe's specific spec structure (1735+ schemas) that `oasdiff` handles inconsistently regardless of how much actually changed. `OASDIFF_TIMEOUT` is 120s, which is not a reliable guarantee even for a modest window, evidently. `watch.yml`'s `timeout-minutes: 20` job guard is the real backstop — a hung run fails safely (checkpoint doesn't advance either way) rather than corrupting anything, just costs an Actions run and retries next cron cycle. **Practical consequence**: this is why the spec-diff detector's actual PR-creation path remains unverified live as of this writing — every attempt at a real, non-mocked end-to-end test has been blocked by `oasdiff` itself, not by this codebase's own logic (which has thorough passing offline tests against real oasdiff output shapes, verified separately). Not going to keep retrying this locally — diminishing returns once the failure mode is this well-characterized; the pipeline will get its real test naturally whenever the scheduled cron happens to hit a live Stripe change with a small, fast diff. A bounded-lookback-window design or a different diff tool would be the real fix; not built, real scope beyond what's been done so far.
...
- **The `MAX_RAW_ENTRIES_PER_RUN` fix above didn't actually converge:** slicing `raw_entries[:300]` positionally re-examined the *identical* prefix every truncated run, since most raw entries don't match a given repo (the real noise-filter is `detect_code_usage`, not the cap) — a low match count didn't mean the window was small, so a real match sitting past position 300 would never be reached. Fixed by tracking `examined_fingerprints` (persisted in `.stripe_spec_cache.json` alongside `checkpoint_sha`/`seen_changes`, cleared once the checkpoint actually advances) so each truncated run skips what it already looked at and makes real progress into the backlog — verified with a test where the first 3 (capped) raw entries are all noise and the real match sits 4th; run 1 finds nothing, run 2 correctly reaches it. A related subtlety caught in a second self-review pass while building this fix: a finding that *matches* but gets cut by the separate `MAX_SPEC_CHANGES_PER_RUN` cap must NOT be marked examined (it hasn't been cached/acted on yet) — only genuinely-resolved fingerprints (no match, or matched-and-kept) get marked, so a cap-cut match still gets a real second chance next run instead of being silently dropped. Also verified with its own test.
  ...
  **Note on review coverage**: that first `/code-review high` run was cut short by a session rate limit before its other angle-finders (correctness, security) reported back — only reuse/simplification/efficiency completed. A second full `/code-review high` pass ran to completion afterward (below) and closed that gap.

  A 5th real `workflow_dispatch` run (after both fixes, `create_prs` checked) went clean end-to-end post-fix: `Loaded 1 cached changes`, `Loaded spec-diff checkpoint ..., 0 cached change(s), 0 examined-but-unresolved fingerprint(s)` (confirming the new state field round-trips correctly), unchanged-checkpoint short-circuit, exit 0, cache saved. The intervening scheduled (cron) run also completed successfully, confirming the unattended loop stayed healthy throughout this whole debugging session.

**A second, complete `/code-review high` pass (2026-08-30) found 5 more findings on the fixes above — 2 fixed, 3 documented (all low-severity/low-likelihood, none currently live):**
- **Fixed — `pr_creator.py`'s fail-closed PR-existence check only caught `GithubException`:** PyGithub does not wrap transient network failures (connection errors, timeouts) in `GithubException` — verified directly against the installed `PyGithub==2.10.0` source. A raw network exception there would have propagated uncaught out of `create_prs_for_fixes`'s loop, aborting every other already-generated fix in that run too, not just the one whose check failed. Now catches bare `Exception`. Also added a batch-level `try/except` around each `create_fix_pr` call in the loop itself (matching `main.py`'s existing per-item isolation pattern), so this class of gap anywhere else in `create_fix_pr` can't take down the whole batch either.
- **Fixed — a raw oasdiff entry with no `fingerprint` could never converge:** `_normalize_oasdiff_entry` already refused to build a change without one, but such an entry stayed in `raw_entries` and could never be added to `examined_fingerprints` (nothing to key it by) — so it would occupy the same wasted slot in `unexamined[:300]` on every truncated retry forever. A narrower instance of the exact non-convergence bug fixed above, for an input oasdiff has always actually provided in every real run seen so far (malformed-input/future-oasdiff-regression guard). Now dropped up front, before the cap/examined-fingerprint bookkeeping, with a warning logged for visibility.
- **Documented, not fixed — the `id` format change has no migration:** changing `id` from `openapi-diff:{sha}:{fingerprint}` to `openapi-diff:{fingerprint}` means any `seen_changes` entries already cached under the old format would never match the new keys, causing a one-time re-detection/re-fix/re-PR for anything matched under the old scheme. Not a live concern for this repo specifically — every real production run's `.stripe_spec_cache.json` has shown `"seen_changes": {}` so far (nothing has ever actually been cached under the old format) — but worth knowing if this code is ever forked/deployed elsewhere with pre-existing cache state.
- **Documented, not fixed — `examined_fingerprints` has no size cap:** during an extended outage, it's persisted and can only grow (cleared only once the checkpoint fully advances) while `latest_sha` keeps moving forward each retry, widening the window. Bounded in practice by the finite size of Stripe's real API surface and the same `timeout-minutes: 20` backstop that already bounds this whole scenario; a real cap would be easy to add if this ever actually becomes a problem, not worth building preemptively for a set of short hash strings.
- **Documented, not fixed — spec-diff's cache-poisoning-by-dry-run gap** (see the "dry run is not read-only" entry above, now extended to cover this detector too).
